...
class _RestaurantsState extends State<Restaurants> {
  ...
  Future getRestaurent() async {
    ...
    if (json['results'] != null) {
      ...
      list = List<RestaurentModel>.from(
          json["results"].map((x) => RestaurentModel.fromJson(x)));
      cuisinesModelList = List<CuisinesModel>.from(
          json["cuisine_list"].map((x) => CuisinesModel.fromJson(x)));
      Provider.of<ApplicationProvider>(context, listen: false)
          .setAllRestaurant(list);
      Provider.of<ApplicationProvider>(context, listen: false)
          .setAllCuisines(cuisinesModelList);
    }
  }
}
        ...
        children: [
          ...
          restModel!.openStatus == "Closed"
              ? Padding(
                  ...
                  child: DottedLine(
                      dashRadius: 5, dashColor: Colors.grey, lineThickness: 1),
                )


              :  restModel!.openStatus == "Busy"
              ? Padding(
              padding: const EdgeInsets.only(top: 5, bottom: 5),
              child: DottedLine(
                  dashRadius: 5, dashColor: Colors.grey, lineThickness: 1))
          ...
          null != restModel!.merchantBranchBusyEndTime &&
                  restModel!.merchantBranchBusy == "1"
              ? Text(
                  "Next available at ${restModel!.restNextAvilableDay.toString() + " " + restModel!.restNextAvilableTime.toString()}",
                  style: TextStyle(
                      fontSize: 12,
